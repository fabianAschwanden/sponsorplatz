...
| **QRB-LAZY-01** | `QrBillServiceLazyIT` | `erzeugeAlsDataUrlFuerId` löst die LAZY-Relation `Rechnung.org` auf, ohne `LazyInitializationException` (echte Ursache BETA-V09: Methode ist `@Transactional(readOnly=true)`, sonst 500 bei `open-in-view=false`) |
...
| **CRM-CTRL-14** | `SponsorAccountControllerTest` | /neu-Formular rendert verstecktes Typ-Feld mit `value="VEREIN"` (sonst sendet Browser typ leer → @Valid scheitert, Formular lädt kommentarlos neu) — Regressionsschutz |
...
| **QRB-05** | `QrBillServiceTest` | QR-IBAN ohne QR-Referenz (`qr_ref_missing`) → `IllegalArgumentException` (400), kein roher 500 |
| **RTPL-01..03** | `RechnungTemplateRenderTest` | Rechnungs-Detail + PDF rendern voll (echtes Thymeleaf/PDF) mit gesetzten String-Feldern + Flash-Erfolgsmeldung — PDF ohne `@environment`-Crash (Sentry-Fragment aus PDF entfernt) |
...
| **ENG-VIEW-01** | `SchaufensterAnsichtTest` | Engagements nach Kanton gruppiert + sortiert, „Übrige Schweiz" zuletzt |
| **ENG-VIEW-02** | `SchaufensterAnsichtTest` | verfügbare Kantone distinct + nach Anzeige sortiert, ohne null |
